...
-- A browser cannot reliably notify the server when a large streamed ZIP is
-- interrupted. Keep the successful-download allowance authoritative, but let
-- the next click replace any unfinished attempt immediately. If the unfinished
-- attempt used a staff grant, return that grant before reserving it again.
...
begin
  ...
  from public.portal_raw_download_attempts
  where portal_id = portal.id and status = 'STARTED' and expires_at > clock_timestamp();
  ...
  return jsonb_build_object(
    'allowed', completed_count < 2 or coalesce(open_request.status in ('GRANTED','RESERVED'), false),
...
begin
  ...
  where r.portal_id = portal.id and r.status = 'RESERVED'
    and exists (
      select 1 from public.portal_raw_download_attempts a
      where a.request_id = r.id and a.status = 'STARTED'
    );
  update public.portal_raw_download_attempts
  set status = 'FAILED', updated_at = clock_timestamp()
  where portal_id = portal.id and status = 'STARTED';
