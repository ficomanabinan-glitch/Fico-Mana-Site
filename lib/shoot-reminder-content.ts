...
export function buildShootReminder(payload: ShootReminderPayload, kind: ShootReminderKind, siteUrl: string) {
  ...
  const details = [
    ...
    ['Shoot time', payload.shootTime || payload.bookingTime],
  ...
  const html = `<div style="background:#f5f5f5;padding:24px 12px;font-family:Arial,sans-serif;color:#222"><div style="max-width:600px;margin:auto;background:white;padding:30px;border-top:4px solid #0500d0">
    ...
    <p style="font-size:13px">All times are Philippine time (Asia/Manila). Please arrive at your scheduled arrival time and bring any outfits or props needed for your session.</p>
    ...
    <p style="line-height:3.5"><a href="${escapeEmailHtml(responseUrl)}?choice=confirmed" style="display:inline-block;background:#0500d0;color:white;text-decoration:none;padding:0 20px;border-radius:6px;margin:4px">Confirm My Shoot</a>
    <a href="${escapeEmailHtml(responseUrl)}?choice=declined" style="display:inline-block;background:#f1f1f1;color:#333;text-decoration:none;padding:0 20px;border-radius:6px;margin:4px">I Can't Attend</a></p>
