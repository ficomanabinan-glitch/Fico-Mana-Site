...
export const onsitePortalEmailSubject = (bookingId: string) =>
  `Your photos are ready to select — FICO MANA ${bookingId}`
...
export async function sendPortalAccessIfNeeded(
  ...
  if (!isValidCustomerEmail(recipient) || isPlaceholderCustomerEmail(recipient)) {
    return {
      sent: false,
      error:
        'The client email is missing or invalid. Try: update it in Booking Management, then retry the email.',
    }
  }
  ...
  if (!portal || portal.status !== 'active') {
    return {
      sent: false,
      error:
        'The client portal is not active. Try: retry provisioning in Client Portals, then retry the email.',
    }
  }
  if (!galleryResult.data?.length) {
    return {
      sent: false,
      error:
        'No uploaded photos are available. Try: finish uploading and Sync Drive before retrying the email.',
    }
  }

  // The photo-selection email never starts or extends the final-gallery access window.
  // Expiry is now created only when the editor marks the finished deliverables DELIVERED.
  ...
  const html = `<div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;padding:24px;border:2px solid #0500D0;border-radius:16px;background:#fff;color:#171717;">
    ...
    <p>Your final-gallery access countdown has not started. It begins only after your editor uploads and releases the finished deliverables.</p>
  ...
  const idempotencyKey = `onsite-portal-${createHash('sha256')
    .update(JSON.stringify([recipient, subject, html]))
    .digest('hex')}`
  ...
  if (!result.success) {
    return {
      sent: false,
      error:
        'The portal email could not be sent. Try: check the client email and the email service settings, then retry.',
    }
  }
