...
import { sendPortalSelectionRejectedEmail } from '@/lib/email'
...
export async function reviewSelection(workspaceId: string, actorId: string, bookingId: string, body: unknown) {
  ...
  if (!input || !['Approve', 'Reject', 'RetryEmail'].includes(String(input.action)) ||
  ...
  if (input.action !== 'RetryEmail') {
    ...
    if (error) {
      if (/Selection changed|not pending review|Editing has already started|not found|Invalid review|not authorized/i.test(error.message)) {
        throw new SelectionReviewError('This selection changed or is no longer pending review. Try: refresh the queue.')
      }
      throw error
    }
  }
  if (input.action === 'Approve') return { success: true }
  // Fetch the committed result, never email a rejection that failed to save.
  const [{ data: booking, error: bookingError }, { data: selection, error: selectionError }, { data: portal, error: portalError }] = await Promise.all([
    ...
    admin.from('client_portals').select('public_id').eq('workspace_id',workspaceId).eq('booking_id',bookingId).single(),
  ])
  ...
  if (booking.raw_photo_status !== 'Rejected' || selection.status !== 'OPEN' ||
  ...
  try {
    const result = await sendPortalSelectionRejectedEmail({
      bookingId, name: String(booking.customer_name), email: String(booking.customer_email),
      reason: String(booking.raw_photo_notes || ''), url: portalUrl(String(portal.public_id)),
    ...
    console.error('Selection rejection email failed:', error)
