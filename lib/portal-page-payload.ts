...
export function portalPagePayload(publicId: string, data: Awaited<ReturnType<typeof getPortalData>>) {
  ...
  return {
    ...
    rawDownloadAllUrl: data.selection?.status === 'SUBMITTED' && data.rawDownloadAccess?.allowed ? `${base}/raw-photos.zip` : null,
    rawDownloadRequestUrl: data.selection?.status === 'SUBMITTED' ? `${base}/raw-download-request` : null,
  }
}
