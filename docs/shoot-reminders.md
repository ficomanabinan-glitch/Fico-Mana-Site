...
- **Notification compatibility:** operational alerts use the existing nullable `notifications.booking_id` with the additive `SHOOT_REMINDER_ERROR` type. The UUID primary key and booking foreign key remain intact; no fake booking or schema migration is required. Staff cannot forge this server-only alert type through the generic notification creation route. Only Fico Mana owner/admin membership can read or dismiss these alerts.
