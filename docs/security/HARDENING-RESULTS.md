...
| FM-03 | Active canonical workspace membership required; no request-time recreation of revoked membership; email bootstrap opt-in only. | Verify existing authorized accounts and MFA recovery before deployment. |
| FM-04 | Admin capabilities through the editor workflow API require verified AAL2 too. | Exercise real admin/editor/onsite sessions against preview and live policy inventory. |
...
| FM-08 | Production requires independent signing/encryption/security keys; malformed AES-GCM envelopes rejected. | Inventory actual deployed keys. Do not rotate keys without portal-link and encrypted Drive-token migration/recovery plans. |
...
| FM-14 | CI actions pinned; checksum-verified full-history Gitleaks added; local reachable-history scan passed after a narrow public-key exception. | Hosted CI execution, repository protections and coordinated PII-history purge are not verified. |
| FM-15 | Existing schema and policies preserved. | Live grants, function search paths/ACLs, RLS and migration installation require privileged read-only verification before any approved forward hardening. |
