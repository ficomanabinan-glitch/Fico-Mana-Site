...
Browser CORS correction: upload-session creation now includes the exact, allowlisted Origin from the authenticated request header (admin/editor/newadmin). It is not taken from JSON and no wildcard is allowed. Previously, the server omitted Origin when it created the Google session; a later browser PUT could be unreadable due to CORS and surface as a generic connection error. This is a code-level finding; the affected real session's browser network log has not been captured because the connected-browser tool is unavailable.
...
- Three file workers share a single page-wide concurrency limit, including different client cards. Each original's chunks remain sequential. Session initialization is serialized per client to avoid racing its first folder setup, while existing transfers continue. Hashing and confirmation occupy a worker too, bounding large-file memory and server verification work.
- Progress shows each active photo and its verification state. A failed file does not count as fully transferred. Retry Failed includes only failed originals; same-client double starts are blocked and duplicate File objects in a selection are ignored. Error text distinguishes an offline browser from an unreadable/blocked Google response without assuming the internet is at fault.
