  ...
  return (
    <div className="mb-6 sm:mb-10 px-1">
      ...
      <div className="hidden sm:flex justify-center gap-2 sm:gap-4 mt-3 flex-wrap px-2">
        {labels.map((l) => (
          <span key={l} className="text-caption sm:text-caption uppercase tracking-wider text-white/40">
            {l}
          </span>
        ))}
      </div>
      <p className="sm:hidden text-center text-caption uppercase tracking-wider text-white/50 mt-2">
            ...
            <div className="flex justify-center gap-2 flex-wrap">
              {(['graduation', 'capping-pinning', 'self-portrait', 'creative'] as const).map((c) => (
                <button
                  ...
                  className={`px-4 py-2 text-caption uppercase tracking-wider border rounded-sm ${
                  ...
                  >
                    ...
                    {pkg.features.length > 0 && (
                      <details className="group relative z-10 border-t border-white/10 mx-0">
                        <summary className="flex cursor-pointer list-none items-center justify-between gap-2 px-5 py-3 text-caption font-semibold uppercase tracking-label text-white/45 hover:text-white/70 select-none [&::-webkit-details-marker]:hidden">
                          <span>Includes</span>
                        ...
                        <ul className="px-5 pb-4 divide-y divide-white/[0.06]">
                          {pkg.features.map((item, i) => (
                            ...
                            >
                              <span className="text-caption font-light tabular-nums text-white/30 w-5 shrink-0 pt-0.5">
                ...
                <div className={`${cardClass} p-4 sm:p-5`}>
                  ...
                  <div className="grid grid-cols-7 gap-1 text-center">
                    {['Su', 'Mo', 'Tu', 'We', 'Th', 'Fr', 'Sa'].map((d) => (
                      <span key={d} className="text-caption font-medium text-white/40 py-1">
                        {d}
                      </span>
                    ))}
                    {cells.map((day, idx) =>
        ...
        {step === 6 && (
          <div className="max-w-md mx-auto text-center space-y-5 py-4">
            ...
            <div className="border border-primary/30 bg-primary/10 p-6 space-y-4 text-left">
              <div>
                <p className="text-caption uppercase tracking-wider text-white/40">Booking Reference</p>
                <p className="text-2xl font-bold text-white font-mono mt-1">{bookingId}</p>
                <button type="button" onClick={() => { navigator.clipboard.writeText(bookingId); setCopiedText(true) }} className="text-caption text-white mt-3 inline-flex items-center gap-1">
              ...
              <div className="border-t border-primary/20 pt-4 space-y-2 text-sm">
                {(submittedSummary?.depositAmount ?? 500) === 0 ? (
                  ...
                  <>
                    <p className="text-caption uppercase tracking-wider text-white/40">Payment Submitted</p>
                    <div className="grid grid-cols-2 gap-3 text-xs">
                      <div>
                      ...
                      <div className="col-span-2">
                        ...
                        {(submittedSummary?.transactionRef || transactionRef.trim()) && (
                          <button
                            ...
                            className="text-caption text-white mt-2 inline-flex items-center gap-1"
                          >
