...
import { ChevronUp, PanelLeftClose, PanelLeftOpen, WalletCards } from 'lucide-react'
  ...
  return <>
    <aside aria-label="Client details, payment summary, and portal QR" tabIndex={0}
      ...
      <button type="button" aria-expanded={!collapsed} aria-label={collapsed ? 'Expand client details' : 'Collapse client details'}
        ...
        className={`flex min-h-10 cursor-pointer items-center justify-center gap-2 rounded-control border border-white/10 bg-white/[0.03] px-3 text-caption font-semibold text-[#C4CEFF] outline-none transition hover:border-[#C4CEFF]/30 hover:bg-white/[0.06] focus-visible:ring-2 focus-visible:ring-[#C4CEFF]/70 ${collapsed ? 'mx-auto size-10 p-0' : 'ml-auto'}`}>
        {collapsed ? <PanelLeftOpen className="size-4" aria-hidden="true" /> : <><span>Collapse</span><PanelLeftClose className="size-4" aria-hidden="true" /></>}
      </button>
