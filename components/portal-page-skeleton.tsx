...
export default function PortalPageSkeleton() {
  return <main className={`client-portal ${styles.portal}`} aria-label="Loading Client Portal" aria-busy="true">
    <header className={styles.topbar}><div className={styles.shell}><div className={styles.identity}><div className="space-y-3"><div className={`${pulse} h-3 w-36`} /><div className={`${pulse} h-7 w-48`} /></div><div className={`${pulse} h-11 w-20`} /></div><div className={styles.navigation}>{Array.from({ length: 4 }, (_, index) => <div className="p-3" key={index}><div className={`${pulse} h-8 w-full`} /></div>)}</div></div></header>
