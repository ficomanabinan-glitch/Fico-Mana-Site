    ...
    <div className="space-y-6 animate-pulse" aria-label="Loading dashboard">
      ...
      <div className="grid lg:grid-cols-12 gap-6">
        <div className="lg:col-span-7 h-[360px] rounded-xl border border-white/10 bg-white/[0.025] p-5">
          <div className="flex justify-between border-b border-white/10 pb-4"><Bar className="h-4 w-44" /><Bar className="h-9 w-28" /></div>
          <div className="space-y-4 pt-5">{Array.from({ length: 5 }).map((_, i) => <Bar key={i} className="h-11 w-full" />)}</div>
        </div>
        <div className="lg:col-span-5 h-[360px] rounded-xl border border-white/10 bg-white/[0.025] p-5">
          <Bar className="h-4 w-36 mb-6" />
          <div className="h-[250px] flex items-end gap-3">{[45,70,38,82,58,92,64].map((h,i)=><div key={i} className="flex-1 rounded-t bg-white/[0.07]" style={{height:`${h}%`}} />)}</div>
        </div>
      </div>
    </div>
  )
...
function BookingsSkeleton() {
  return (
    <div className="space-y-6 animate-pulse" aria-label="Loading bookings">
      <HeaderSkeleton />
      <div className="rounded-xl border border-white/10 bg-white/[0.025] p-4 grid sm:grid-cols-2 lg:grid-cols-5 gap-3">
        <Bar className="h-10 lg:col-span-2" />
        <Bar className="h-10" /><Bar className="h-10" /><Bar className="h-10" />
      </div>
      <div className="rounded-xl border border-white/10 bg-white/[0.025] overflow-hidden">
        ...
        {Array.from({ length: 7 }).map((_, i) => (
          <div key={i} className="min-h-16 border-b last:border-b-0 border-white/[0.06] px-4 grid grid-cols-[1.2fr_1fr_1fr_.9fr_.8fr] gap-4 items-center">
            {Array.from({length:5}).map((_,j)=><Bar key={j} className={`h-4 ${j===0?'w-4/5':'w-full'}`} />)}
          </div>
        ))}
      </div>
    </div>
    ...
    <div className="space-y-6 animate-pulse" aria-label="Loading verification queue">
      ...
      <div className="grid lg:grid-cols-2 gap-5">
        {Array.from({length:4}).map((_,i)=>(
          <div key={i} className="rounded-xl border border-white/10 bg-white/[0.025] p-5 grid grid-cols-[120px_1fr] gap-5 min-h-[210px]">
            <div className="rounded-lg bg-white/[0.07]" />
            <div className="space-y-3"><Bar className="h-5 w-28"/><Bar className="h-4 w-3/4"/><Bar className="h-4 w-1/2"/><Bar className="h-9 w-full mt-6"/></div>
          </div>
        ))}
      </div>
    </div>
  ...
  return (
    <div className="space-y-6 animate-pulse" aria-label="Loading calendar">
      ...
      <div className="grid xl:grid-cols-[minmax(280px,340px)_minmax(0,1fr)] gap-6 items-start">
        <div className="rounded-xl border border-white/10 bg-white/[0.025] p-4 h-[430px]">
          <div className="flex justify-between mb-5"><Bar className="h-4 w-28"/><Bar className="h-8 w-20"/></div>
          <div className="grid grid-cols-7 gap-2">{Array.from({length:42}).map((_,i)=><div key={i} className="aspect-square rounded bg-white/[0.06]" />)}</div>
        </div>
        <div className="space-y-5">
          <div className="h-[190px] rounded-xl border border-white/10 bg-white/[0.025] p-5 space-y-4"><Bar className="h-5 w-44"/><Bar className="h-10 w-full"/><Bar className="h-10 w-3/4"/></div>
          <div className="h-[300px] rounded-xl border border-white/10 bg-white/[0.025] p-5 space-y-4"><Bar className="h-5 w-40"/>{Array.from({length:4}).map((_,i)=><Bar key={i} className="h-12 w-full" />)}</div>
        </div>
      </div>
    </div>
  )
...
function EmailsSkeleton() {
  return (
    <div className="space-y-6 animate-pulse" aria-label="Loading email logs">
      <HeaderSkeleton />
      <div className="rounded-xl border border-white/10 bg-white/[0.025] p-4"><Bar className="h-11 w-full" /></div>
      <div className="rounded-xl border border-white/10 bg-white/[0.025] overflow-hidden divide-y divide-white/[0.06]">
        {Array.from({length:7}).map((_,i)=><div key={i} className="p-5 flex items-center justify-between gap-5 min-h-[92px]"><div className="flex-1 space-y-2"><Bar className="h-3 w-2/3"/><Bar className="h-4 w-1/2"/><Bar className="h-3 w-32"/></div><Bar className="h-9 w-24"/></div>)}
      </div>
    </div>
  )
}
...
function FilteringSkeleton() {
  return (
    <div className="space-y-6 animate-pulse" aria-label="Loading filtering dashboard">
      <HeaderSkeleton />
      <div className="rounded-xl border border-white/10 bg-white/[0.025] p-1.5 flex gap-1">
        {[110,130,105,90].map((w,i)=><Bar key={i} className="h-10" />)}
      </div>
      <div className="grid sm:grid-cols-2 xl:grid-cols-4 gap-4">
        {Array.from({length:4}).map((_,i)=><div key={i} className="h-28 rounded-xl border border-white/10 bg-white/[0.025] p-4 space-y-3"><Bar className="h-3 w-24"/><Bar className="h-7 w-16"/><Bar className="h-3 w-32"/></div>)}
      </div>
      <div className="grid lg:grid-cols-[1.15fr_.85fr] gap-6">
        <div className="h-[340px] rounded-xl border border-white/10 bg-white/[0.025] p-5 space-y-4"><Bar className="h-5 w-44"/>{Array.from({length:5}).map((_,i)=><Bar key={i} className="h-12 w-full" />)}</div>
        <div className="h-[340px] rounded-xl border border-white/10 bg-white/[0.025] p-5 space-y-4"><Bar className="h-5 w-36"/><Bar className="h-11 w-full"/>{Array.from({length:4}).map((_,i)=><Bar key={i} className="h-10 w-full" />)}</div>
      </div>
    </div>
  )
}
