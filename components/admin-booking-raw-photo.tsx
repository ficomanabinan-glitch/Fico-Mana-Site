  ...
  return (
    <div className="border border-white/10 p-4 space-y-3">
      <div className="flex items-center justify-between border-b border-white/10 pb-1.5 gap-2">
        <h4 className="text-caption font-semibold tracking-label text-white/40 uppercase flex items-center gap-1.5">
      ...
      {!canSubmit && !submitted && (
        <p className="text-caption text-white/45 leading-relaxed">
      ...
      {canSubmit && (
        <div className="space-y-2">
          <p className="text-caption text-white/45">Send this link so the client can submit name + Drive folder:</p>
          <div className="flex gap-2">
            <input
              readOnly
              value={submitUrl}
              className="flex-1 min-w-0 bg-black/40 border border-white/10 px-2 py-1.5 text-caption font-mono text-white/70"
            />
            <button
              type="button"
              onClick={copySubmitLink}
              className="shrink-0 px-3 py-1.5 border border-white/10 bg-white/5 hover:bg-white/10 text-caption font-semibold uppercase tracking-wider flex items-center gap-1"
            >
      ...
      {submitted ? (
        <div className="space-y-2 text-xs">
          <div className="flex items-center justify-between gap-2">
            ...
            <span
              className={`px-2 py-0.5 text-caption font-semibold uppercase rounded border ${rawPhotoStatusBadgeClass(status || 'Pending Review')}`}
            >
          ...
          {booking.rawPhotoNotes && (
            <p className="text-caption p-2 bg-white/[0.03] border border-white/10 italic text-white/70">
              {booking.rawPhotoNotes}
            </p>
          )}
        </div>
      ) : canSubmit ? (
        <p className="text-caption text-amber-400/90">Waiting for client to submit a raw photo link.</p>
      ) : null}
    </div>
  )
}
