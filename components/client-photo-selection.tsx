...
export function ClientPhotoSelection({
  ...
  return <section className="border border-white/10 bg-white/[0.02] p-4 sm:p-6">
    ...
    <label className="mt-5 block rounded-xl border border-white/[0.08] bg-black/10 p-4">
      <span className="text-[10px] font-bold uppercase tracking-wider text-[#C4CEFF]">Editing preference</span>
      ...
      <span className="mt-2 block text-[10px] text-white/35">{locked && !editingPreference ? 'Your previously submitted preferences are preserved.' : 'Applies to all included photos and Extra Edits.'}</span>
      {editingPreference === 'standard' ? <span className="mt-1 block text-[10px] text-white/25">See our posted samples on our Social Media.</span> : null}
    </label>
    ...
    {step === 'prints' ? <div className="mt-5 space-y-4">
      ...
      {PRINTS.map((print) => {
        ...
        return <div key={print.category} className="block rounded-xl border border-white/[0.08] bg-black/10 p-4">
          <label htmlFor={`print-${print.category}`} className="text-[10px] font-bold uppercase tracking-wider text-[#C4CEFF]">{print.label}</label>
          <p className="mt-1 text-[10px] text-white/35">Choose {print.quantity} cop{print.quantity === 1 ? 'y' : 'ies'}.</p>
          ...
          {file ? <button type="button" onClick={() => setPreviewFile(file)} aria-label={`Preview ${print.label}: ${file.fileName}`} className="mt-3 flex cursor-pointer items-center gap-3 rounded-lg border border-white/10 bg-black/10 p-2 text-left transition hover:border-[#C4CEFF]/40">
            ...
            <span className="min-w-0"><span className="block break-all text-[10px] text-white/65">{file.fileName}</span><span className="mt-1 flex items-center gap-1 text-[10px] text-[#C4CEFF]"><ZoomIn className="size-3"/>View photo · {print.quantity} cop{print.quantity === 1 ? 'y' : 'ies'}</span></span>
          </button> : null}
        </div>
      })}
