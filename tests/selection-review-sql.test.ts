...
async function fixture() {
  ...
  await db.exec(`
    ...
    create table editing_jobs(id uuid primary key default gen_random_uuid(),workspace_id uuid,booking_id varchar,status text,downloaded_at timestamptz,download_lock_expires_at timestamptz,last_error text,updated_at timestamptz);
...
test('approval unlocks editing only after staff review; repeated approve is idempotent and contradictory reject blocked',async t => {
  ...
  await assert.rejects(f.review('Reject'),/not pending review/)
  assert.equal((await f.db.query('select * from workflow_audit_logs')).rows.length,1)
})
