...
test('desktop contact sheet keeps preview and mobile selection as separate actions', () => {
  ...
  assert.match(gallery,/selectionOrder=\{selectionOrder\.get\(file\.id\)\}/)
  assert.match(gallery,/Extra \$\{money\(extraPrice\)\}/)
  assert.doesNotMatch(gallery,/`\+\$\{money\(extraPrice\)\}`/)
  assert.match(gallery,/className=\{styles\.previewChoice\}/)
...
test('approved photo layout keeps identity and navigation in one sticky header without restoring a sidebar', () => {
  ...
  assert.ok(css.includes('grid-template-columns:minmax(0,1.35fr) minmax(380px,1fr)'))
  assert.ok(css.includes('grid-template-columns:repeat(5,minmax(0,1fr))'))
  assert.ok(css.includes('.filters { display:grid; grid-template-columns:repeat(2,minmax(82px,1fr))'))
  assert.ok(css.includes('--portal-header-height'))
  assert.match(css,/\.preview \{ position:sticky; top:calc\(var\(--portal-header-height\) \+ 20px\); align-self:start; max-height:calc\(100dvh - var\(--portal-header-height\) - 20px\)/)
...
test('portal workflow navigation is compact, single-line and keeps accessible touch targets', () => {
  ...
  assert.match(selection, /<span>\{item\.label\}<\/span>/)
  assert.doesNotMatch(selection, /<small>/)
  assert.match(css, /\.step\[aria-current=step\] \{[^}]*color:white; background:#0500d0;[^}]*box-shadow:/)
