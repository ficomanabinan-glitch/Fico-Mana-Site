...
test('new admin aliases are exact-host-only and never redirect the original consoles', () => {
  ...
  for (const path of ['/admin','/admin/bookings','/api/bookings','/auth/callback','/_next/static/a.js','/newadmin','/newadmin/bookings']) assert.equal(newAdminAlias('newadmin.ficomana.com',path),null)
})

test('the new console requires canonical administrator access and reuses existing auth and APIs', () => {
  ...
  assert.doesNotMatch(layout,/assurance|aal2|\/admin\/mfa/i)
...
test('presentation examples stay synthetic and grouping preserves whole-week/month keys', () => {
  ...
  assert.ok(example.batches.every(b => b.id.startsWith('SAMPLE-')))
