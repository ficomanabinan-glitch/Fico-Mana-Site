...
test('isolated semantic color pairs meet WCAG AA including muted, status, focus and chart states', () => {
  const css = source('components/new-admin/new-admin.module.css')
  const colors = Object.fromEntries([...css.matchAll(/--([a-z-]+): (#[a-f0-9]{6});/g)].map(match => [match[1],match[2]]))
  ...
  assert.ok(contrast('#ffffff','#3a3c40') >= 4.5)
  assert.match(css,/--leading-golden: 1.618/)
