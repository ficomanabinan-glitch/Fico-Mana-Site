...
test('Client Portals uses the available width and keeps Portal/QR actions without the redundant Copy button', async()=> {
  ...
  assert.ok(page.includes('xl:grid-cols-[minmax(0,1fr)_minmax(360px,430px)]'),'storage details and policy share one compact desktop row')
  assert.ok(page.includes('sm:flex-row sm:items-end'),'the expiry field and Save policy action align vertically when there is room')
  assert.ok(page.includes('sm:w-auto sm:min-w-32'),'Save policy stays compact on desktop')
  assert.match(page,/const actionButton = .*rounded-control/,'Client Portal actions use the durable rounded control token')
