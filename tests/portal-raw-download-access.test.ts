...
test('database policy completes two downloads, queues a reason, and consumes one staff grant', async () => {
  ...
  const abandoned = await db.query<{ result: { attemptId: string } }>(`select begin_portal_raw_download($1,$2) result`, [workspace, publicId])
  const retryState = await db.query<{ state: { allowed: boolean; activeDownloads: number } }>(`select portal_raw_download_state($1,$2) state`, [workspace, publicId])
  assert.equal(retryState.rows[0].state.allowed, true)
  assert.equal(retryState.rows[0].state.activeDownloads, 1)
