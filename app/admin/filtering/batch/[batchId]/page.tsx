...
export default function BatchDetailPage() {
  ...
  return <div className="space-y-6">
    ...
    {detail.jobs.some((job)=>job.enhancementPreferences.length||job.printAllocations.length||job.addonOrders.length)?<section className={`${adminPanel} overflow-hidden`}><div className="border-b border-white/[0.08] p-4"><p className="text-[9px] font-bold uppercase tracking-wider text-[#C4CEFF]">Selection Instructions</p><h2 className="mt-1 text-sm font-semibold">Enhancement, free prints, and paid add-ons</h2><p className="mt-1 text-[10px] text-white/35">Editors can follow each client&apos;s final choices without opening another screen.</p></div><div className="divide-y divide-white/[0.06]">{detail.jobs.filter((job)=>job.enhancementPreferences.length||job.printAllocations.length||job.addonOrders.length).map((job)=><SelectionInstructions key={job.bookingId} job={job} busy={busyJob===job.bookingId} canAdmin={canAdmin} onClientStatus={updateClientStatus}/>)}</div></section>:null}
...
function SelectionInstructions({job,busy,canAdmin,onClientStatus}:{job:Job;busy:boolean;canAdmin:boolean;onClientStatus:(bookingId:string,status:ClientSelectionStatus)=>Promise<void>}) {
  ...
  return <article className="p-4 sm:p-5">
    <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
      <div><p className="text-sm font-semibold">{job.customerName}</p><p className="mt-1 text-[10px] text-white/35">{job.packageName} · {job.bookingId}</p></div>
      {canAdmin?<label className="min-w-48"><span className="block text-[8px] font-bold uppercase tracking-wider text-white/30">Client workflow status</span><select value={job.selectionClientStatus} disabled={busy} onChange={(event)=>void onClientStatus(job.bookingId,event.target.value as ClientSelectionStatus)} className="mt-1 w-full cursor-pointer rounded-lg border border-white/10 bg-[#222222] px-3 py-2 text-[10px] text-white outline-none focus:border-[#C4CEFF]/50 disabled:cursor-not-allowed">{CLIENT_SELECTION_STATUSES.map((status)=><option key={status} value={status}>{status}</option>)}</select></label>:<span className="w-fit rounded border border-[#C4CEFF]/20 bg-[#C4CEFF]/[0.06] px-2.5 py-1 text-[9px] font-bold uppercase text-[#C4CEFF]">{job.selectionClientStatus}</span>}
    </div>
    <div className="mt-4 grid gap-3 lg:grid-cols-3">
      <div className="rounded-xl border border-white/[0.07] bg-black/10 p-4"><p className="text-[8px] font-bold uppercase tracking-wider text-white/30">Enhancement Preferences</p><div className="mt-3 space-y-2">{job.enhancementPreferences.map((item)=><div key={item.fileId} className="border-b border-white/[0.05] pb-2 text-[10px] last:border-0 last:pb-0"><div className="flex items-center gap-2"><span className="min-w-0 flex-1 truncate font-semibold text-white/70" title={item.fileName}>{item.fileName}</span>{item.extraEdit?<span className="shrink-0 rounded bg-amber-500/10 px-1.5 py-0.5 text-[8px] font-bold uppercase text-amber-300">Extra Edit</span>:null}</div><p className="mt-1 text-white/35">{preferenceLabel(item.preference)}</p></div>)}</div></div>
      <div className="rounded-xl border border-white/[0.07] bg-black/10 p-4"><p className="text-[8px] font-bold uppercase tracking-wider text-white/30">Free Print Allocation</p><div className="mt-3 space-y-2">{job.printAllocations.map((item)=><div key={item.category} className="border-b border-white/[0.05] pb-2 text-[10px] last:border-0 last:pb-0"><p className="font-semibold text-[#C4CEFF]">{item.label}</p><p className="mt-1 truncate text-white/35" title={item.fileName}>{item.fileName} · Qty {item.quantity}</p></div>)}</div></div>
      <div className="rounded-xl border border-white/[0.07] bg-black/10 p-4"><p className="text-[8px] font-bold uppercase tracking-wider text-white/30">Paid Add-ons</p><div className="mt-3 space-y-2">{job.addonOrders.length?job.addonOrders.map((item,index)=><div key={`${item.name}-${index}`} className="flex items-start justify-between gap-3 border-b border-white/[0.05] pb-2 text-[10px] last:border-0 last:pb-0"><div><p className="font-semibold text-white/70">{item.name}</p><p className="mt-1 text-white/30">Qty {item.quantity}{item.photoCount?` · ${item.photoCount} photo${item.photoCount===1?'':'s'}`:''}</p></div><span className="shrink-0 font-semibold text-[#C4CEFF]">{money(item.total)}</span></div>):<p className="text-[10px] text-white/30">No paid add-ons.</p>}</div><div className="mt-3 flex items-center justify-between border-t border-white/[0.07] pt-3 text-[10px]"><span className="text-white/35">Add-on total</span><strong className="text-sm text-[#C4CEFF]">{money(job.totalAddonAmount)}</strong></div></div>
    </div>
  </article>
}
