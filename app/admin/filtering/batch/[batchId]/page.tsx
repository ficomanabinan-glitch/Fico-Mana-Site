...
export default function BatchDetailPage() {
  ...
  return <div className="space-y-6">
    ...
    <div className="grid grid-cols-2 gap-px overflow-hidden border border-white/[0.08] bg-white/[0.06] sm:grid-cols-4 xl:grid-cols-8">{stats.map(([label,value])=><div key={label} className="bg-[#222222] p-4"><p className="text-caption font-semibold uppercase tracking-wider text-white/25">{label}</p><p className={`mt-1 text-xl font-bold ${label==='Failed'&&value>0?'text-red-300':''}`}>{value}</p></div>)}</div>
...
function ProgressStat({label,value}:{label:string;value:string}) { return <div className="border border-white/[0.07] bg-black/10 p-3"><p className="text-caption font-semibold uppercase tracking-wider text-white/25">{label}</p><p className="mt-1 truncate text-sm font-semibold">{value}</p></div> }
