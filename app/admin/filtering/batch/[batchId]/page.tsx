...
type WorkflowMember = { id:string;role:string;displayName:string;email:string }
...
export default function BatchDetailPage() {
  ...
  const load=async()=>{
    try {
      const [response,sessionResponse,membersResponse]=await Promise.all([
        ...
        fetch('/api/editor-workflow/members',{cache:'no-store',credentials:'include'}),
      ])
      ...
      if(membersResponse.ok){const memberBody=await membersResponse.json() as {members?:WorkflowMember[]};setMembers(memberBody.members||[])}
  ...
  const assignJob=async(bookingId:string,assigneeId:string|null)=>{
    setBusyJob(bookingId)
    try{
      const response=await fetch(`/api/editor-workflow/jobs/${encodeURIComponent(bookingId)}/assign`,{method:'POST',credentials:'include',headers:{'Content-Type':'application/json'},body:JSON.stringify({assigneeId})})
      const body=await responseJson(response);if(!response.ok)throw new Error(String(body.error||'Could not assign this job.'))
      toast.success(assigneeId?'Editor assigned':'Assignment cleared',assigneeId?'The job is now protected from accidental duplicate work.':'The job is available to claim.');await load()
    }catch(error){toast.error('Assignment failed',error instanceof Error?error.message:'Try again.')}
    finally{setBusyJob('')}
  }
...
function JobRow({job,busy,session,members,canOnsite,canEdit,canAdmin,onUploadRaw,onSyncRaw,onReconcile,onAssign,onReopen,onStatus}:{job:Job;busy:boolean;session:WorkflowSession|null;members:WorkflowMember[];canOnsite:boolean;canEdit:boolean;canAdmin:boolean;onUploadRaw:(bookingId:string)=>void;onSyncRaw:(bookingId:string)=>Promise<void>;onReconcile:(bookingId:string,repair?:boolean)=>Promise<void>;onAssign:(bookingId:string,assigneeId:string|null)=>Promise<void>;onReopen:(bookingId:string)=>Promise<void>;onStatus:(bookingId:string,status:JobStatus)=>Promise<void>}){
