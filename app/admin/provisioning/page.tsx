...
export default function ProvisioningPage() {
  ...
  return <div className="w-full min-w-0 space-y-6 font-sans" data-testid="client-portals-page">
    ...
    <section className={`${adminPanel} space-y-5 p-5 md:p-6`}>
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start"><Cloud className="mt-0.5 size-5 shrink-0 text-[#C4CEFF]" /><div className="min-w-0 flex-1"><h2 className="text-sm font-semibold">Private photo storage</h2><p className="mt-1 text-sm text-white/65">Cloudflare R2 · Private bucket · Temporary signed access</p><p role="status" className={`mt-2 text-sm ${!storage ? 'text-white/65' : storage.configured ? 'text-green-300' : 'text-amber-200'}`}>{!storage ? (loading ? 'Checking storage configuration…' : 'Storage configuration could not be loaded. Use Sync to try again.') : storage.configured ? 'Storage credentials are configured on the server.' : 'Storage credentials are missing. Uploads and downloads remain disabled.'}</p></div>{storage ? <ShieldCheck className={`size-5 ${storage.configured ? 'text-emerald-300' : 'text-amber-300'}`} /> : <LoaderCircle aria-hidden="true" className="size-5 animate-spin text-white/65" />}</div>
