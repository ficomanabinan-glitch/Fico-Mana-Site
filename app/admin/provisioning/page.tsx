...
export default function ProvisioningPage() {
  ...
  const [overview, setOverview, loading, setLoading, refreshing] = useCachedPageRead<Overview | null>('admin:provisioning', null)
  ...
  return <div className="w-full min-w-0 space-y-6 font-sans" data-testid="client-portals-page">
    ...
    <section className={`${adminPanel} space-y-5 p-5 md:p-6`}>
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start"><Cloud className="mt-0.5 size-5 shrink-0 text-[#C4CEFF]" /><div className="flex-1"><h2 className="text-sm font-semibold">Private photo storage</h2><p className="mt-1 text-xs text-white/45">Cloudflare R2 · Private bucket · Temporary signed access</p><p className={`mt-2 text-xs ${storage?.configured ? 'text-green-300' : 'text-amber-200'}`}>{storage?.configured ? 'Storage credentials are configured on the server.' : 'Storage credentials are missing. Uploads and downloads remain disabled.'}</p></div><ShieldCheck className={`size-5 ${storage?.configured ? 'text-emerald-300' : 'text-amber-300'}`} /></div>
      <div className="grid items-end gap-3 sm:grid-cols-[minmax(0,240px)_auto]"><label className="flex flex-col gap-1.5"><span className="text-caption uppercase tracking-wider text-white/40">Portal expiry days</span><input type="number" min={1} max={3650} value={expiryDays} onChange={event => { settingsRevision.current++; setExpiryDays(event.target.value) }} className={adminInput} /><span className="text-caption leading-relaxed text-white/35">Applies after the first successful portal-ready email.</span></label><button disabled={saving} onClick={() => void saveSettings()} className={`${actionButton} min-h-10 px-4`}>{saving ? 'Saving…' : 'Save policy'}</button></div>
    </section>
