...
const portalSecondaryAction='transition-all duration-300 ease-out hover:-translate-y-0.5 hover:border-[#C4CEFF]/45 hover:bg-[#C4CEFF]/[0.07] hover:text-white hover:shadow-[0_8px_24px_rgba(0,0,0,0.28)] active:translate-y-0 active:scale-[0.99] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#C4CEFF]/60 focus-visible:ring-offset-2 focus-visible:ring-offset-[#171717] disabled:pointer-events-none disabled:translate-y-0 disabled:shadow-none'
  ...
  return <main className="min-h-screen bg-[#171717] text-white"><div className="mx-auto max-w-6xl px-4 py-8 sm:px-6 sm:py-12">
    ...
    <div className="mt-6 grid gap-5 lg:grid-cols-3"><section className="space-y-5 lg:col-span-2">
      ...
      <section className="border border-white/10 bg-white/[0.02] p-5 sm:p-6"><div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><div className="flex items-center gap-2"><ImageIcon className="size-4 text-[#C4CEFF]"/><h2 className="text-sm font-semibold">Your Photos</h2></div><p className="mt-2 text-xs leading-relaxed text-white/45">Choose exactly {required||'the configured number of'} photo{required===1?'':'s'} for editing. Full-resolution studio RAW files are never exposed publicly.</p></div>{data.selection?<div className={`shrink-0 border px-3 py-2 text-center ${locked?'border-emerald-500/20 bg-emerald-500/10':'border-[#C4CEFF]/20 bg-[#C4CEFF]/5'}`}><p className="text-[9px] uppercase tracking-wider text-white/35">Photo Selection</p><p className={`mt-1 text-sm font-bold ${locked?'text-emerald-300':'text-[#C4CEFF]'}`}>{selected.length} / {required} Selected</p></div>:null}</div>
        ...
        {data.gallery.length===0?<div className="mt-5 border border-white/[0.07] bg-black/10 p-8 text-center text-xs text-white/35">Your studio gallery is still being prepared.</div>:<div className="mt-5 grid grid-cols-2 gap-3 sm:grid-cols-3 xl:grid-cols-4">{data.gallery.map((file)=>{const active=selected.includes(file.id);return <button key={file.id} type="button" disabled={locked} onClick={()=>toggle(file.id)} className={`group relative overflow-hidden border text-left transition-all duration-300 ease-out hover:-translate-y-1 hover:shadow-[0_14px_32px_rgba(0,0,0,0.38)] active:translate-y-0 active:scale-[0.99] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#C4CEFF]/70 ${active?'border-[#C4CEFF] ring-2 ring-[#C4CEFF]/25':'border-white/10 hover:border-[#C4CEFF]/45'} ${locked?'cursor-default disabled:pointer-events-none':''}`}><div className="aspect-[4/5] overflow-hidden bg-black/20"><img src={file.previewUrl} alt={file.fileName} loading="lazy" className="h-full w-full object-cover transition-transform duration-500 ease-out group-hover:scale-[1.035]"/></div><div className="flex items-center justify-between gap-2 bg-[#1d1d1d] p-2 transition-colors duration-300 group-hover:bg-[#252525]"><span className="truncate text-[9px] text-white/45 transition-colors group-hover:text-white/70">{file.fileName}</span>{active?<span className="flex size-5 shrink-0 items-center justify-center rounded-full bg-[#C4CEFF] text-black"><Check className="size-3"/></span>:null}</div></button>})}</div>}
        {data.gallery.length<data.galleryTotal?<button onClick={()=>void load(data.gallery.length)} disabled={loadingMore} className={`mt-4 w-full border border-white/10 px-4 py-3 text-[10px] font-bold uppercase text-white/60 disabled:opacity-40 ${portalSecondaryAction}`}>{loadingMore?'Loading more…':`Load More Photos (${data.gallery.length} / ${data.galleryTotal})`}</button>:null}
        {!locked&&data.gallery.length>0?<div className="mt-5 flex flex-col gap-3 border-t border-white/[0.07] pt-5 sm:flex-row sm:items-center sm:justify-between"><p className="text-[11px] text-white/40">{selected.length<required?`Choose ${required-selected.length} more photo${required-selected.length===1?'':'s'}.`:'Your selection is complete. Submit when ready.'}</p><button onClick={()=>void submit()} disabled={!canSubmit||submitting} className={`${portalPrimaryAction} px-5 py-3 text-[10px] font-bold uppercase tracking-wider disabled:opacity-35`}>{submitting?'Submitting…':'Submit Selection'}</button></div>:null}
      </section>
